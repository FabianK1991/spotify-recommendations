function createFetchPolyfill() {
  ...
  let support = {
    searchParams: 'URLSearchParams' in win,
    iterable: 'Symbol' in win && 'iterator' in Symbol,
    blob: 'FileReader' in win && 'Blob' in win && (function() {
      try {
        new Blob();
        return true;
      } catch(e) {
    ...
    formData: 'FormData' in win,
    arrayBuffer: 'ArrayBuffer' in win
  };
  ...
  retObj.fetch = function(input, init) {
    return new Promise(function(resolve, reject) {
  ...
  retObj.fetch.polyfill = true;

  return retObj;
}

export default createFetchPolyfill().fetch;